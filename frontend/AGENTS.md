...
- `src/components/` — UI 컴포넌트 (`erd/` ERD 뷰어, `DatabaseConnect/` 연결 온보딩, `InputPanel/`, `AuditDrawer/`, `DiffControls/`, `StageBadge/`, `StageProgress/`, `CompletedBar/`, `DiagnosticsPanel/` 무결성 진단, `ModelSettings/` LLM 선택·다운로드, `LanguageToggle/`, `AppBackdrop/`, `AppliedToast/`)
