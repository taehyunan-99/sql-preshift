...
- `package.json` — Electron + electron-builder + dev 편의(concurrently/wait-on). 패키징 `build` 키(3d). `build.mac.icon` → `build/icon.icns`.
- `build/icon.icns` — 앱 아이콘(Safe-Gate Shield, `assets/icon/`에서 iconutil로 생성). `.gitignore`의 `build/` 규칙 예외로 추적([[app-icon-safe-gate-shield]]).
...
- dmg를 재빌드했다고 sidecar 검증을 건너뛰지 말 것 — 백엔드 의존성 중 방언/플러그인을 런타임 동적 import하는 라이브러리(sqlglot 등)는 PyInstaller 정적 분석이 못 잡아 번들에서 누락될 수 있다. docker 개발 경로에는 정상 설치돼 있어 이 결함이 안 보이고, 설치형 앱에서만 `/api/analyze`가 500으로 죽어 안전 게이트가 통째로 무력화된다(커밋 594bde3에서 실측). sidecar/spec 변경 후에는 패키징 바이너리를 직접 실행해 analyze e2e까지 확인한다([[pyinstaller-sqlglot-dialects]], backend COMMANDS 가드 참고).
